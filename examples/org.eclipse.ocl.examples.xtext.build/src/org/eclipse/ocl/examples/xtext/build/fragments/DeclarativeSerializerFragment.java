...
import org.eclipse.ocl.examples.xtext.build.xtext.ParserRuleAnalysis;
...
public abstract class DeclarativeSerializerFragment extends SerializerFragment2
{
	...
	protected @NonNull String getSerializationSegmentsId(@NonNull List<SerializationSegment> segments, boolean addQualifier) {
	...
	protected @NonNull Iterable<@NonNull List<SerializationSegment>> getSerializationSegmentsIterable(@NonNull GrammarAnalysis grammarAnalysis) {
		...
		for (@NonNull Idiom idiom : grammarAnalysis.getIdioms()) {
			...
			for(@NonNull SubIdiom subIdiom : staticSubIdioms) {
				...
				List<@NonNull SerializationSegment> serializationSegments = new ArrayList<>(segments.size());
				for (Segment segment : segments) {
					assert segment != null;
					SerializationSegment serializationSegment = grammarAnalysis.getSerializationSegment(segment);
					serializationSegments.add(serializationSegment);
				}
				if (serializationSegments.size() > 0) {
					segments2id2.put(serializationSegments, "");
				}
			}
		}
	...
	protected @NonNull Iterable<@NonNull SerializationSegment[][]> getSerializationSegmentsListIterable(@NonNull GrammarAnalysis grammarAnalysis) {
	...
	protected @NonNull String getSerializationSegmentsListString(@NonNull SerializationSegment[][] segmentsList) {
