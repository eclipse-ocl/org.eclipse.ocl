...
public abstract class GenerateLPGUtils extends GenerateLPG
{
	...
	protected String emitContinueEObject(Conjunction conjunction) {
		String setAttributes = "SetAttributes";
					...
					if (ePackage !=  null) {
						...
						if (eClassifier instanceof EClass) {
							...
							if ((eStructuralFeature != null) && isMapFeature(eStructuralFeature)) {
								setAttributes = "SetMapAttributes";
							}
						}
					}
				}
		...
		case 1: return setAttributes + ".create(getRhsSym(1))";
		case 2: return setAttributes + ".concatenate(getRhsSym(1), getRhsSym(2))";
		}
		return null;
	}
		...
		if (rule != null) {
			...
			if (segmentsLength >= 1) {
				...
				if ("STRING".equals(algorithm)) {
					...
					return emitCreateEcoreFeature(rule, segments);
				}
				if (segmentsLength >= 3) {
				}
			}
	...
	private String emitCreateEcoreClass(@NonNull Conjunction conjunction, @NonNull String @NonNull [] segments) {
		...
		s.append("createEcoreClass(" + packageInterfaceName + ".Literals." + CodeGenUtil.upperName(className) + "__" + CodeGenUtil.upperName(featureName));
		List<AbstractElement> elements = conjunction.getElements();
		for (int i = 0; i < elements.size(); i++) {
			AbstractElement element = elements.get(i);
			if (element instanceof RuleCall) {
				s.append(", ");
				s.append("getRhsSym(" + (i+1) + ")");
			}
		}
		s.append(")");
		return s.toString();
	...
	private String emitCreateEcoreRoot(@NonNull Conjunction conjunction, @NonNull String @NonNull [] segments) {
		...
		String packageInterfaceName = getEPackageInterfaceName(ePackage);
		StringBuilder s = new StringBuilder();
		s.append("createEcoreRoot(" + packageInterfaceName + ".Literals." + CodeGenUtil.upperName(className));
		List<AbstractElement> elements = conjunction.getElements();
		for (int i = 0; i < elements.size(); i++) {
			AbstractElement element = elements.get(i);
			if (element instanceof RuleCall) {
	...
	private String emitCreateEcoreFeature(@NonNull AbstractRule rule, @NonNull String @NonNull [] segments) {
		int segmentsLength = segments.length;
		String packageName = segments[segmentsLength-3];
		String className = segments[segmentsLength-2];
		String featureName = segments[segmentsLength-1];
		...
		if (ePackage !=  null) {
			String packageInterfaceName = getEPackageInterfaceName(ePackage);
			return "createEcoreFeature(" + packageInterfaceName + ".Literals." + CodeGenUtil.upperName(className) + "__" + CodeGenUtil.upperName(featureName) + ")";
		}
		return null;
	}
