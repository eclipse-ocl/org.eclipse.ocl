...
public class CopyAndRename extends AbstractWorkflowComponent2 {
	...
	private String fromRawProjectFolder;
	private String fromProjectFolder;
	private String script;
	private String toProjectFolder;
	 ...
	 */
	public void setFromProjectFolder(final String fromProjectFolder) {
		this.fromRawProjectFolder = fromProjectFolder;
		this.fromProjectFolder = locate(fromProjectFolder);
	}
	 ...
	 */
	public void setToProjectFolder(final String toProjectFolder) {
		this.toProjectFolder = locate(toProjectFolder);
	}
...
//				}
	...
	private void copyFile(File fromFile, File toFile) {
		...
		try {
			...
			if (editedText != null) {
				...
				String fromFileName = fromRawProjectFolder + fromFile.toString().replace("\\",  "/").substring(fromProjectFolder.length());
				editedText =
						"/**\n" +
	...
	private String locate(String projectFolder) {
		int index = projectFolder.indexOf("/");
		String projectName = index >= 0 ? projectFolder.substring(0, index) : projectFolder;
		String folderPath = index >= 0 ? projectFolder.substring(index+1) : "";
		Map<String, URI> platformResourceMap = EcorePlugin.getPlatformResourceMap();
		URI uri = platformResourceMap.get(projectName);
		if (uri != null) {
			if (uri.isFile()) {
				return uri.toFileString() + folderPath;
			}
			else {
				return uri.toString() + folderPath;
			}
		}
		else {
