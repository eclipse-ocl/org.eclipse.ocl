...
//	private final @NonNull Map<@NonNull Property, @NonNull CGProperty> cgProperties = new HashMap<>();
	 ...
	 */
	private @NonNull Map<@NonNull Constraint, @NonNull CGConstraint> asConstraint2cgConstraint = new HashMap<>();

	/**
	 * Mapping from each AS LoopExp to its corresponding CGIterationCallExp.
	 */
	private @NonNull Map<@NonNull LoopExp, @NonNull CGIterationCallExp> asLoopExp2cgIterationCallExp = new HashMap<>();
	...
	public void addCGIterationCallExp(@NonNull CGIterationCallExp cgIterationCallExp) {			// private
		LoopExp asLoopExp = (LoopExp)CGUtil.getAST(cgIterationCallExp);
		CGIterationCallExp old = asLoopExp2cgIterationCallExp.put(asLoopExp, cgIterationCallExp);
		assert old == null;
	}
...
//				classNameManager.declarePreferredName(cgOperation);
				pushOperationNameManager(cgOperation);
...
//					nameManager.declarePreferredName(cgAccumulator);
		...
		if (iterationHelper != null) {			// Helper: iterators are part of invocation context
			initAst(cgIterationCallExp, asLoopExp);
			pushIterateNameManager(cgIterationCallExp);
		}
		//
		//	Body
		//
...
//	}
	...
	public @NonNull Operation getNativeOperation(@NonNull Method method, @NonNull OperationCallingConvention callingConvention) {
		...
		if (cgOperation == null) {
			...
			cgNativeOperation.setAst(asOperation);
			pushOperationNameManager(cgNativeOperation);
...
//				nameManager.declarePreferredName(cgParameter);
	...
	public @NonNull Property getNativeProperty(@NonNull Field field, @NonNull PropertyCallingConvention callingConvention) {
		...
		if (cgProperty == null) {
			...
			cgNativeProperty.setAst(asProperty);
			pushPropertyNameManager(cgNativeProperty);
			popNestedNameManager();
			popClassNameManager();
		}
		return asProperty;
	...
	protected @NonNull ClassNameManager lazyGetClassNameManager(org.eclipse.ocl.pivot.@NonNull Class asClass) {
		CGClass cgClass = basicGetCGClass(asClass);
		if (cgClass == null) {
	...
	public @NonNull FeatureNameManager pushConstraintNameManager(@NonNull CGConstraint cgConstraint) {
	...
	public @NonNull NestedNameManager pushOperationNameManager(@NonNull CGOperation cgOperation) {
		Operation asOperation = CGUtil.getAST(cgOperation);
		...
		org.eclipse.ocl.pivot.Class asClass = PivotUtil.getOwningClass(asOperation);
		ClassNameManager classNameManager = lazyGetClassNameManager(asClass);
	...
	public @NonNull FeatureNameManager pushPropertyNameManager(@NonNull CGProperty cgProperty) {
		Property asProperty = CGUtil.getAST(cgProperty);
	...
	public @NonNull FeatureNameManager useFeatureNameManager(@NonNull TypedElement asTypedElement) {	// OCLExpression or ExpressionInOCL
		for (EObject eObject = asTypedElement; eObject != null; eObject = eObject.eContainer()) {
			if (eObject instanceof Constraint) {
				...
				CGIterationCallExp cgIterationCallExp = basicGetCGIterationCallExp((LoopExp)eObject);
