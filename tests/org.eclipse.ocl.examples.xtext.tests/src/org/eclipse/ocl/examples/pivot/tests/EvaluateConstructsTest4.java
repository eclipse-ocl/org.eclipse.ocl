...
public class EvaluateConstructsTest4 extends PivotTestSuite
{
	...
	/*	@Test public void testConstruct_xx() throws Exception {
		TestOCL ocl = createOCL();
		@NonNull String expectedResultExpression = "Sequence{1.0,1.0,1.0}";
		@NonNull String expression = "let s = Sequence{0.0,0.0,0.0} in s->collect(r | r + Sequence{1.0,2.0,3.0}->at(s->indexOf(r)))";
		Object expectedResultQuery = ocl.evaluateLocal(null, expectedResultExpression);
		Object expectedValue = expectedResultQuery instanceof Value ? expectedResultQuery : ocl.getIdResolver().boxedValueOf(expectedResultQuery);
		LibraryUnaryOperation.LibraryUnaryOperationExtension testInstance = new EvaluateConstructs.TestClass0(); //(LibraryUnaryOperation.LibraryUnaryOperationExtension) genModelHelper.loadClass(query, targetFolder, packageName, className, true);
		Executor executor = new EcoreExecutorManager(ocl, PivotTables.LIBRARY);
		TypeId typeId = TypeId.SEQUENCE.getSpecializedId(TypeId.REAL);
		Object value = testInstance.evaluate(executor, typeId, ocl);
		ocl.assertOCLEquals(expression, expectedValue, value);
		//		PivotTestSuite.appendLog(ocl.testName, context, expression, null, expectedValue != null ? expectedValue.toString() : null, null);
		//		PivotTestSuite.appendLog(ocl.testName, context, expression, null, expectedResultExpression, null);
		ocl.dispose();
	} */
}
