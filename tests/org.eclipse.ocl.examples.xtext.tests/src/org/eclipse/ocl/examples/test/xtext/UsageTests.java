...
public class UsageTests extends PivotTestSuite// XtextTestCase
{
	...
	private static final class TestUMLImporter
		...
		public void reloadGenModel(@NonNull Path genModelPath)
				throws Exception {
	...
	public @NonNull TestOCL createOCL() {
		return new TestOCL(getTestFileSystem(), getTestPackageName(), getName(),
			getTestProjectManager(), null);
	}
	...
	protected @NonNull URI getModelsURI(@NonNull String filePath) {
		return URI.createPlatformResourceURI(
			getTestBundleName() + "/models/" + filePath, true);
	}
	...
	public static @NonNull String createClassPath(
			@NonNull List<String> projectNames) {
			...
			if (projectPath != null) {
				...
				if (projectPath.endsWith("/")) {
					projectPath = projectPath
						+ JavaFileUtil.TEST_BIN_FOLDER_NAME;
				}
	...
	public @NonNull String createEcoreGenModelContent(@NonNull String fileName,
			@Nullable Map<@NonNull String, @Nullable String> genOptions)
			throws Exception {
		String interfacePackageSuffix = genOptions != null
			? genOptions.get("interfacePackageSuffix")
			: null;
		String metaDataPackageSuffix = genOptions != null
			? genOptions.get("metaDataPackageSuffix")
			: null;
		String usedGenPackages = genOptions != null
			? genOptions.get("usedGenPackages")
			: null;
		String modelDirectory = genOptions != null
			? genOptions.get("modelDirectory")
			: null;
		String dynamicTemplates = genOptions != null
			? genOptions.get("dynamicTemplates")
			: null;
		String templateDirectory = genOptions != null
			? genOptions.get("templateDirectory")
			: null;
		if (modelDirectory == null) {
			modelDirectory = getTestProject().getName() + "/"
				+ JavaFileUtil.TEST_SRC_FOLDER_NAME;
		}
		StringBuilder s = new StringBuilder();
		...
		s.append(
			"platform:/resource/org.eclipse.ocl.pivot/model/oclstdlib.genmodel#//oclstdlib\"\n");
	...
	public void createManifestFile()
		...
		TestFile testFile = getTestProject()
			.getOutputFile("META-INF/MANIFEST.MF");
		...
		writer.append(
			"Bundle-SymbolicName: " + bundleName + ";singleton:=true\n");
		writer.append("Bundle-Version: 1.0.0.qualifier\n");
		writer.close();
	}

	protected @NonNull URI createModels(@NonNull String testFileStem,
			@Nullable String oclinecoreFile, @NonNull String genmodelContent)
			throws Exception {
		...
		try {
			...
			URI genModelURI = createTestFileWithContent(
				getTestProject().getOutputFile(testFileStem + ".genmodel"),
				genmodelContent);
	...
	public @NonNull URI createTestFileWithContent(@NonNull TestFile testFile,
			String fileContent)
			throws IOException {
		File file = testFile.getFile();
		Writer writer = new FileWriter(file);
	...
	/**
	 * Create a seed *.ecore from the *.uml profile with correct genPackage
	 * declarations s that a reload / reconcile corrects the genClasses etc.
	 */
	public @NonNull String createUMLEcoreModelContent(
			@NonNull Resource umlProfileResource)
			throws Exception {
		StringBuilder s = new StringBuilder();
		s.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
		...
		for (EObject eRootObject : umlProfileResource.getContents()) {
			if (eRootObject instanceof Model) {
				for (org.eclipse.uml2.uml.Package umlPackage : ((Model) eRootObject)
					.getNestedPackages()) {
					assert umlPackage != null;
					appendEcorePackage(s, umlPackage);
				}
			} else if (eRootObject instanceof org.eclipse.uml2.uml.Package) {
				appendEcorePackage(s,
					(org.eclipse.uml2.uml.Package) eRootObject);
			}
		}
		s.append("</xmi:XMI>\n");
		return s.toString();
	...
	private void appendEcorePackage(@NonNull StringBuilder s,
			org.eclipse.uml2.uml.@NonNull Package umlPackage) {
		Stereotype ecoreStereotype = umlPackage
			.getAppliedStereotype("Ecore::EPackage");
		String basePackage = (String) umlPackage.getValue(ecoreStereotype,
			"basePackage");
		String nsPrefix = (String) umlPackage.getValue(ecoreStereotype,
			"nsPrefix");
		String nsURI = (String) umlPackage.getValue(ecoreStereotype, "nsURI");
		String packageName = (String) umlPackage.getValue(ecoreStereotype,
			"packageName");
		String prefix = (String) umlPackage.getValue(ecoreStereotype, "prefix");
		s.append("  <ecore:EPackage");
		s.append(" name=\"" + (packageName != null
			? packageName
			: umlPackage.getName()) + "\"");
		if (basePackage != null) {
	...
	public @NonNull String createUMLGenModelContent(
			@NonNull Resource umlProfileResource, @NonNull String fileName,
			@Nullable Map<@NonNull String, @Nullable String> genOptions)
			throws Exception {
		String interfacePackageSuffix = genOptions != null
			? genOptions.get("interfacePackageSuffix")
		...
		s.append(
			"    <details key=\"Use Null Annotations\" value=\"true\"/>\n");
		s.append("  </genAnnotations>\n");
		s.append("  <foreignModel>" + umlProfileResource.getURI().lastSegment()
			+ "</foreignModel>\n");
		for (EObject eRootObject : umlProfileResource.getContents()) {
			if (eRootObject instanceof Model) {
				for (org.eclipse.uml2.uml.Package umlPackage : ((Model) eRootObject)
					.getNestedPackages()) {
					assert umlPackage != null;
					appendGenModelPackage(s, umlPackage, interfacePackageSuffix,
						metaDataPackageSuffix, fileName);
				}
			} else if (eRootObject instanceof org.eclipse.uml2.uml.Package) {
				appendGenModelPackage(s,
					(org.eclipse.uml2.uml.Package) eRootObject,
					interfacePackageSuffix, metaDataPackageSuffix, fileName);
			}
		}
		if (usedGenPackages != null) {
			for (String usedGenPackage : usedGenPackages.split(" ")) {
				s.append(
					"  <usedGenPackages href=\"" + usedGenPackage + "\"/>\\n");
			}
		}
		s.append(
			"  <usedGenPackages href=\"platform:/resource/org.eclipse.emf.ecore/model/Ecore.genmodel#//ecore\"/>\n");
		s.append(
			"  <usedGenPackages href=\"platform:/resource/org.eclipse.uml2.types/model/Types.genmodel#//types\"/>\n");
		s.append(
			"  <usedGenPackages href=\"platform:/resource/org.eclipse.uml2.uml/model/UML.genmodel#//uml\"/>\n");
		// s.append(" <usedGenPackages
		// href=\"platform:/resource/org.eclipse.ocl.pivot/model/oclstdlib.genmodel#//oclstdlib\"/>\n");
		s.append("</genmodel:GenModel>\n");

		return s.toString();
	}

	private void appendGenModelPackage(StringBuilder s,
			org.eclipse.uml2.uml.@NonNull Package umlPackage,
			String interfacePackageSuffix, String metaDataPackageSuffix,
			@NonNull String fileStem) {
		String packageName = umlPackage.getName();
		s.append("  <genPackages xsi:type=\"genmodel:GenPackage\" prefix=\""
			+ packageName + "\"\n");
		s.append("    disposableProviderFactory=\"true\"\n");
	...
	protected boolean doCompile(@NonNull OCL ocl,
			@NonNull JavaClasspath classpath,
			@NonNull String... testProjectNames)
			throws Exception {
		List<@NonNull JavaFileObject> compilationUnits = new ArrayList<>();
		StringBuilder sources = new StringBuilder();
		if (testProjectNames != null) {
			for (@NonNull
			String testProjectName : testProjectNames) {
				TestFile srcTestFile = getTestProject().getOutputFile(
					JavaFileUtil.TEST_SRC_FOLDER_NAME + "/" + testProjectName);
				JavaFileUtil.gatherCompilationUnits(compilationUnits,
					srcTestFile.getFile());
				if (sources.length() > 0) {
					sources.append(", ");
				}
		...
		String objectPath = getTestProject()
			.getOutputFile(JavaFileUtil.TEST_BIN_FOLDER_NAME + "/")
			.getFileString();
		classpath.addClass(getClass());
		String problemMessage = JavaFileUtil.compileClasses(compilationUnits,
			sources.toString(), objectPath, classpath);
	...
	protected boolean doEcoreCompile(@NonNull OCL ocl,
			@NonNull String... testProjectNames)
			throws Exception {
	...
	public abstract class OCLTestThread<R> extends OCLThread<R>
	{
	...
	protected void doGenModel(@NonNull URI genmodelURI) throws Throwable {
		OCLTestThread<?> reconcileThread = new OCLTestThread<Object>("GenModel-Reconcile")
		{
			...
			public Object runWithThrowable() {
				...
				URI fileURI = genmodelURI; // getProjectFileURI(testFileStem +
											// ".genmodel");
				...
				// ocl.dispose();
				projectMap.configure(resourceSet,
				...
				resourceSet.getPackageRegistry().put(
					org.eclipse.uml2.codegen.ecore.genmodel.GenModelPackage.eNS_URI,
					org.eclipse.uml2.codegen.ecore.genmodel.GenModelPackage.eINSTANCE);
				resourceSet.getResourceFactoryRegistry().getExtensionToFactoryMap()
				 ...
				 * resourceSetImpl = (ResourceSetImpl) resourceSet; Map<URI,
				 * Resource> uriResourceMap = resourceSetImpl.getURIResourceMap();
				 * if (uriResourceMap != null) { uriResourceMap.clear(); } }
				 */
				...
				if (!(eObject instanceof GenModel)) {
					throw new ConfigurationException(
						"No GenModel found in '" + resource.getURI() + "'");
				}
		...
		genmodelThread.syncExec();
	}

	protected EPackage doLoadPackage(@NonNull ExplicitClassLoader classLoader,
			@NonNull String qualifiedModelPackageName)
			throws Exception {
	...
	protected void doUMLCompile(@NonNull TestOCL ocl,
			@NonNull String testProjectName)
			throws Exception {
		JavaClasspath classpath = JavaFileUtil.createDefaultOCLClasspath();
	...
	protected void initializeResourceSet() {
		...
		// ArrayList<Resource>(resourceSet.getResources());
	}

	protected @NonNull Resource loadUmlProfile(@NonNull ResourceSet resourceSet,
			@NonNull URI umlProfileURI) {
		UMLResourcesUtil.init(resourceSet);
		Resource umlProfileResource = resourceSet.getResource(umlProfileURI,
			true);
	...
	protected @NonNull Resource validateUmlModel(@NonNull URI umlModelURI,
			@NonNull String qualifiedPackageClassName,
			@NonNull String pathMapName, @Nullable Map<URI, URI> extraUriMap)
		...
		Class<?> packageImplClass = urlClassLoader
			.loadClass(qualifiedPackageClassName);
		...
		EPackage.Registry.INSTANCE.remove(nsURI); // We don't need it in the
													// global registry and
													// another test may get a
													// CCE if same nsURI.
		//
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				String oclinecoreFile = "package bug370824 : bug370824 = 'http://bug370824'\n"
					...
					+ "    }\n" + "}\n";
				String genmodelFile = createEcoreGenModelContent("Bug370824",
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				String oclinecoreFile = "package bug409650 : bug409650 = 'http://bug409650'\n"
					...
					+ "    }\n" + "}\n";
				String genmodelFile = createEcoreGenModelContent("Bug409650",
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
				...
				TestOCL ocl = createOCL();
				doEcoreCompile(ocl, testProjectName);
				String qualifiedPackageName = testProjectName + "."
					+ testFileStem + "Package";
				File classFilePath = getTestProject()
					.getOutputFolder(JavaFileUtil.TEST_BIN_FOLDER_NAME + "/")
					.getFile();
				List<@NonNull String> packagePaths = JavaFileUtil
					.gatherPackageNames(classFilePath, null);
				ExplicitClassLoader classLoader = new ExplicitClassLoader(
					classFilePath, packagePaths, getClass().getClassLoader());
				EPackage ePackage = doLoadPackage(classLoader,
				...
				ocl.assertQueryEquals(eObject, "XtestingY",
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				String oclinecoreFile = "import ecore : 'http://www.eclipse.org/emf/2002/Ecore#/';\n"
					...
					+ "             };" + "        }\n" + "    }\n" + "}\n";
				String genmodelFile = createEcoreGenModelContent(testFileStem,
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				String oclinecoreFileA = "package bug416421A : bug416421A = 'example.org/bug416421A'\n"
					...
					+ "			body: false;\n" + "		}\n" + "	}\n"
					+ "}\n";
				String genmodelFileA = createEcoreGenModelContent(testFileStemA,
					null);
				...
				genOptions.put("usedGenPackages",
					"Bug416421A.genmodel#//bug416421A");
				String genmodelFileB = createEcoreGenModelContent(testFileStemB,
					genOptions);
				// doDelete(testProjectNameA);
				// doDelete(testProjectNameB);
				URI genModelURIA = createModels(testFileStemA, oclinecoreFileA,
					genmodelFileA);
				URI genModelURIB = createModels(testFileStemB, oclinecoreFileB,
					genmodelFileB);
				// B first demonstrates the demand load of Bug416421A to fix Bug
				// 416421
	...
	public void testBug567919() throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				EReference ePlan_backlog = (EReference) ePlanClass
					.getEStructuralFeature("backlog");
				EAttribute ePlan_derivedMinSprintCount = (EAttribute) ePlanClass
					.getEStructuralFeature("derivedMinSprintCount");
				EClass eBacklogClass = (EClass) ePackage
					.getEClassifier("Backlog");
				EReference eBacklog_workItems = (EReference) eBacklogClass
					.getEStructuralFeature("workItems");
				EClass eWorkItemClass = (EClass) ePackage
					.getEClassifier("WorkItem");
				EAttribute eWorkItem_effort = (EAttribute) eWorkItemClass
					.getEStructuralFeature("effort");
				EFactory eFactory = ePackage.getEFactoryInstance();
				...
				List<EObject> castWorkItems = (List<EObject>) backlog
					.eGet(eBacklog_workItems);
	...
	/*
	 * FIXME Bug 569113 public void testBug569113() throws Exception { String
	 * testFileStem = "Bug569113"; String testProjectName = "bug569113"; String
	 * oclinecoreFile =
	 ...
	 * "             body : eInts->sum();\n" + "        }\n" + "    }\n" +
	 * "}\n"; String genmodelFile = createEcoreGenModelContent(testFileStem,
	 * null); createManifestFile(); URI genModelURI = createModels(testFileStem,
	 * oclinecoreFile, genmodelFile); doGenModel(genModelURI); TestOCL ocl =
	 * createOCL(); doEcoreCompile(ocl, testProjectName); ocl.dispose(); }
	 */
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				try {
					...
					TestFile umlProfileFile = getTestFile(
						testFileStem + ".profile.uml", ocl0, getTestModelURI(
							"models/uml/" + testFileStem + ".profile.uml"));
					umlProfileResource = loadUmlProfile(resourceSet0,
						umlProfileFile.getURI());
					String ecoreFileContent = createUMLEcoreModelContent(
						umlProfileResource);
					String genmodelFileContent = createUMLGenModelContent(
						umlProfileResource, testFileStem, null);
					createManifestFile();
					createTestFileWithContent(getTestProject().getOutputFile(
						testFileStem + ".profile.ecore"), ecoreFileContent);
					genModelURI = createTestFileWithContent(
						getTestProject()
							.getOutputFile(testFileStem + ".profile.genmodel"),
						genmodelFileContent);
					Path genModelPath = new Path(
						"/" + getTestProject().getName() + "/" + testFileStem
							+ ".profile.genmodel");
					//
					TestUMLImporter importer = new TestUMLImporter(
						resourceSet0.getPackageRegistry());
	 ...
	 * and compiled.
	 */
	public void testBug570717a_uml()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				TestFile umlProfileFile = getTestFile(
					testFileStem + ".profile.uml", ocl0, getTestModelURI(
						"models/uml/" + testFileStem + ".profile.uml"));
				Resource umlProfileResource = loadUmlProfile(
					ocl0.getResourceSet(), umlProfileFile.getURI());
				String ecoreFileContent = createUMLEcoreModelContent(
					umlProfileResource);
				String genmodelFileContent = createUMLGenModelContent(
					umlProfileResource, testFileStem, null);
				...
				URI genModelURI = createTestFileWithContent(
					getTestProject()
						.getOutputFile(testFileStem + ".profile.genmodel"),
					genmodelFileContent);
				Path genModelPath = new Path("/" + getTestProject().getName()
					+ "/" + testFileStem + ".profile.genmodel");
				//
				TestUMLImporter importer = new TestUMLImporter(
					ocl0.getResourceSet().getPackageRegistry());
				importer.reloadGenModel(genModelPath);
				ocl0.dispose();
				//
				doGenModel(genModelURI);
	...
	public void testBug570802()
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				String genmodelFile = createEcoreGenModelContent("Bug570802",
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
	 ...
	 * and compiled.
	 */
	public void testBug570891_uml()
			throws Throwable {
		if (OCLGenModelUtil.INSTANCE.hasDoubleOverrideBug547424()) { // Avoid
																		// UML
																		// BUG
																		// 547424
			System.err
				.println(getName() + " has been disabled -see UML Bug 547424");
			return;
		}
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				// TestFile umlModelFile = getTestFile(testFileStem + ".uml",
				// ocl0, getTestModelURI("models/uml/" + testFileStem +
				// ".uml"));
				TestFile umlProfileFile = getTestFile(
					testFileStem + ".profile.uml", ocl0, getTestModelURI(
						"models/uml/" + testFileStem + ".profile.uml"));
	...
	public void testBug570892_uml()
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				TestFile umlModelFile = getTestFile(testFileStem + ".uml", ocl1,
					getTestModelURI("models/uml/" + testFileStem + ".uml"));
				TestFile umlProfileFile = getTestFile(
					testFileStem + ".profile.uml", ocl1, getTestModelURI(
						"models/uml/" + testFileStem + ".profile.uml"));
				Resource umlProfileResource = loadUmlProfile(
					ocl1.getResourceSet(), umlProfileFile.getURI());
				String ecoreFileContent = createUMLEcoreModelContent(
					umlProfileResource);
				String genmodelFileContent = createUMLGenModelContent(
					umlProfileResource, testFileStem, null);
				...
				//
				TestUMLImporter importer = new TestUMLImporter(
					ocl1.getResourceSet().getPackageRegistry());
				importer.reloadGenModel(genModelPath);
				ocl1.dispose();
				//
				doGenModel(genModelURI);
				...
				Resource umlModelResource = validateUmlModel(
					umlModelFile.getURI(), qualifiedPackageClassName,
					pathMapName, null);
				assertEquals("AbstractEnvironmentFactory.CONSTRUCTION_COUNT", 0,
					AbstractEnvironmentFactory.CONSTRUCTION_COUNT
						- oldAbstractEnvironmentFactory_CONSTRUCTION_COUNT);
				assertEquals("AbstractModelManager.CONSTRUCTION_COUNT", 1,
					AbstractModelManager.CONSTRUCTION_COUNT
						- oldAbstractModelManager_CONSTRUCTION_COUNT);
				assertEquals("ExecutorManager.CONSTRUCTION_COUNT", 1,
					ExecutorManager.CONSTRUCTION_COUNT
						- oldExecutorManager_CONSTRUCTION_COUNT);
				assertEquals("AbstractExecutor.CONSTRUCTION_COUNT", 0,
					AbstractExecutor.CONSTRUCTION_COUNT
						- oldAbstractExecutor_CONSTRUCTION_COUNT);
	...
	/**
	 * Verify that the static profile in Bug570894.profile.uml model can be
	 * generated and compiled and that the Bug570894.uml model can then
	 * validate.
	 */
	public void testBug570894_uml()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				TestFile umlModelFile = getTestFile(testFileStem + ".uml", ocl1,
					getTestModelURI("models/uml/" + testFileStem + ".uml"));
				TestFile umlLibraryFile = getTestFile(
					testFileStem + ".library.uml", ocl1, getTestModelURI(
						"models/uml/" + testFileStem + ".library.uml"));
				TestFile umlProfileFile = getTestFile(
					testFileStem + ".profile.uml", ocl1, getTestModelURI(
						"models/uml/" + testFileStem + ".profile.uml"));
				Resource umlProfileResource = loadUmlProfile(
				...
				extraUriMap.put(URI.createURI(
					"pathmap://VALIDATIONPROBLEM_LIBRARY/ValidationProblem-Library.uml"),
					umlLibraryFile.getURI());
				extraUriMap.put(URI.createURI(
					"pathmap://VALIDATIONPROBLEM_PROFILE/ValidationProblem.profile.uml"),
					umlProfileFile.getURI());
				Resource umlModelResource = validateUmlModel(
					umlModelFile.getURI(), qualifiedPackageClassName,
					pathMapName, extraUriMap);
				assertEquals("AbstractEnvironmentFactory.CONSTRUCTION_COUNT", 0,
					AbstractEnvironmentFactory.CONSTRUCTION_COUNT
						- oldAbstractEnvironmentFactory_CONSTRUCTION_COUNT);
				assertEquals("AbstractModelManager.CONSTRUCTION_COUNT", 1,
				...
				assertEquals("AbstractExecutor.CONSTRUCTION_COUNT", 0,
					AbstractExecutor.CONSTRUCTION_COUNT
						- oldAbstractExecutor_CONSTRUCTION_COUNT);
				for (@NonNull
				EObject eObject : umlModelResource.getContents()) {
					EClass eClass = eObject.eClass();
					if ("Farm".equals(eClass.getName())) {
						EStructuralFeature animalFeature = eClass
							.getEStructuralFeature("animal");
						EStructuralFeature chickenFeature = eClass
							.getEStructuralFeature("chicken");
						EStructuralFeature duckFeature = eClass
							.getEStructuralFeature("duck");
	...
	public void testBug571407_uml()
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				// TestFile umlModelFile = getTestFile(testFileStem + ".uml",
				// ocl1, getTestModelURI("models/uml/" + testFileStem +
				// ".uml"));
				// TestFile umlLibraryFile = getTestFile(testFileStem +
				// ".library.uml", ocl1, getTestModelURI("models/uml/" +
				// testFileStem + ".library.uml"));
				TestFile umlProfileFile = getTestFile(
					testFileStem + ".profile.uml", ocl1, getTestModelURI(
						"models/uml/" + testFileStem + ".profile.uml"));
				Resource umlProfileResource = loadUmlProfile(
				...
				//
				/*
				 * int oldAbstractEnvironmentFactory_CONSTRUCTION_COUNT =
				 * AbstractEnvironmentFactory.CONSTRUCTION_COUNT; int
				 * oldAbstractModelManager_CONSTRUCTION_COUNT =
				 * AbstractModelManager.CONSTRUCTION_COUNT; int
				 * oldExecutorManager_CONSTRUCTION_COUNT =
				 * ExecutorManager.CONSTRUCTION_COUNT; int
				 * oldAbstractExecutor_CONSTRUCTION_COUNT =
				 * AbstractExecutor.CONSTRUCTION_COUNT; // Execute the profile
				 * String qualifiedPackageClassName =
				 * "Bug570894.validationproblem.ValidationProblemPackage";
				 * String pathMapName = "pathmap://VALIDATIONPROBLEM_PROFILE/";
				 * Map<URI, URI> extraUriMap = new HashMap<URI, URI>();
				 * extraUriMap.put(URI.createURI(
				 * "pathmap://VALIDATIONPROBLEM_LIBRARY/ValidationProblem-Library.uml"
				 * ), umlLibraryFile.getURI()); extraUriMap.put(URI.createURI(
				 * "pathmap://VALIDATIONPROBLEM_PROFILE/ValidationProblem.profile.uml"
				 * ), umlProfileFile.getURI()); Resource umlModelResource =
				 * validateUmlModel(umlModelFile.getURI(),
				 * qualifiedPackageClassName, pathMapName, extraUriMap);
				 * assertEquals("AbstractEnvironmentFactory.CONSTRUCTION_COUNT",
				 * 0, AbstractEnvironmentFactory.CONSTRUCTION_COUNT-
				 * oldAbstractEnvironmentFactory_CONSTRUCTION_COUNT);
				 * assertEquals("AbstractModelManager.CONSTRUCTION_COUNT", 1,
				 * AbstractModelManager.CONSTRUCTION_COUNT-
				 * oldAbstractModelManager_CONSTRUCTION_COUNT);
				 ...
				 * assertEquals("AbstractExecutor.CONSTRUCTION_COUNT", 0,
				 * AbstractExecutor.CONSTRUCTION_COUNT-
				 * oldAbstractExecutor_CONSTRUCTION_COUNT); for (@NonNull
				 * EObject eObject : umlModelResource.getContents()) { EClass
				 * eClass = eObject.eClass(); if
				 * ("Farm".equals(eClass.getName())) { EStructuralFeature
				 * animalFeature = eClass.getEStructuralFeature("animal");
				 * EStructuralFeature chickenFeature =
				 * eClass.getEStructuralFeature("chicken"); EStructuralFeature
				 * duckFeature = eClass.getEStructuralFeature("duck"); Object
				 * animals = eObject.eGet(animalFeature); assertEquals(4,
				 * ((Collection<?>)animals).size());
				 *
				 * @SuppressWarnings("unused") Object chicken =
				 * eObject.eGet(chickenFeature);
				 *
				 * @SuppressWarnings("unused") Object duck =
				 * eObject.eGet(duckFeature); } } ocl2.dispose();
				 */
			}
		});
	}
	...
	public void testCSE()
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				TestOCL ocl = createOCL();
				doEcoreCompile(ocl, testProjectName);
				String qualifiedPackageName = testProjectName + ".coreM."
					+ testFileStem + "Package";
				File classFilePath = getTestProject()
					.getOutputFolder(JavaFileUtil.TEST_BIN_FOLDER_NAME + "/")
					.getFile();
				...
				// org.eclipse.ocl.pivot.Class contextType =
				// helper.getOCL().getMetamodelManager().getType(idResolver.getStaticTypeOf(eObject));
	...
	public void testCodegenCompany()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				URI sourceGenModelURI = getTestModelURI(
					"models/genmodel/CodeGenCompany.genmodel");
				...
				try {
					...
					// CommonSubexpressionEliminator.CSE_REWRITE.setState(true);
					//
					Resource genModelResource = ocl0.getResourceSet()
						.getResource(sourceGenModelURI, true);
					GenModel genModel = (GenModel) genModelResource
						.getContents().get(0);
					genModel.setModelDirectory(getTestProject().getName() + "/"
						+ JavaFileUtil.TEST_SRC_FOLDER_NAME);
	...
	public void testEcoreLists570717()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				// + " attribute eNumber : ENumber { derived readonly volatile }
				// { derivation: negENumber(ENumber{'1'}); }\n"
				...
				// + " operation negENumber(b : ENumber) : ENumber { body:
				// (-(b.oclAsType(Integer))).oclAsType(ENumber); }\n"
					...
					+ "    }\n" + "}\n";
				String genmodelFile = createEcoreGenModelContent("Bug570717",
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
				...
				try {
					...
					String qualifiedPackageName = testProjectName + "."
						+ testFileStem + "Package";
					File classFilePath = getTestProject().getOutputFolder(
						JavaFileUtil.TEST_BIN_FOLDER_NAME + "/").getFile();
					List<@NonNull String> packagePaths = JavaFileUtil
						.gatherPackageNames(classFilePath, null);
					ExplicitClassLoader classLoader = new ExplicitClassLoader(
						classFilePath, packagePaths,
						getClass().getClassLoader());
					EPackage ePackage = doLoadPackage(classLoader,
						qualifiedPackageName);
					EClass eClass = (EClass) ePackage
						.getEClassifier("EcoreLists");
					EFactory eFactory = ePackage.getEFactoryInstance();
					//
					EObject eObject = eFactory.create(eClass);
					// FIXME BUG 570800 ocl.assertQueryTrue(eObject, "eChars =
					// Set{eChar}");
					// FIXME BUG 570800 ocl.assertQueryTrue(eObject,
					// "negEChars(eChars) = Set{negEChar(eChar)}");
					// FIXME BUG 570800 ocl.assertQueryTrue(eObject,
					// "eCharacterObjects = Set{eCharacterObject}");
					// FIXME BUG 570800 ocl.assertQueryTrue(eObject,
					// "negECharacterObjects(eCharacterObjects) =
					// Set{negECharacterObject(eCharacterObject)}");
					ocl.assertQueryTrue(eObject,
						"eBigDecimals = Set{eBigDecimal}");
					ocl.assertQueryTrue(eObject,
						"negEBigDecimals(eBigDecimals) = Set{negEBigDecimal(eBigDecimal)}");
					ocl.assertQueryTrue(eObject,
						"eBigIntegers = Set{eBigInteger}");
					ocl.assertQueryTrue(eObject,
						"negEBigIntegers(eBigIntegers) = Set{negEBigInteger(eBigInteger)}");
					ocl.assertQueryTrue(eObject, "eBoolean = eBooleanObject");
					ocl.assertQueryTrue(eObject, "eBooleans = eBooleanObjects");
					ocl.assertQueryTrue(eObject,
						"notEBooleans(eBooleans) = Set{notEBoolean(eBoolean)}");
					ocl.assertQueryTrue(eObject,
						"eBooleanObjects = Set{eBooleanObject}");
					ocl.assertQueryTrue(eObject,
						"notEBooleanObjects(eBooleanObjects) = Set{notEBooleanObject(eBooleanObject)}");
					ocl.assertQueryTrue(eObject, "eDoubles = Set{eDouble}");
					ocl.assertQueryTrue(eObject,
						"negEDoubles(eDoubles) = Set{negEDouble(eDouble)}");
					ocl.assertQueryTrue(eObject,
						"eDoubleObjects = Set{eDoubleObject}");
					ocl.assertQueryTrue(eObject,
						"negEDoubleObjects(eDoubleObjects) = Set{negEDoubleObject(eDoubleObject)}");
					ocl.assertQueryTrue(eObject, "eFloats = Set{eFloat}");
					ocl.assertQueryTrue(eObject,
						"negEFloats(eFloats) = Set{negEFloat(eFloat)}");
					ocl.assertQueryTrue(eObject,
						"eFloatObjects = Set{eFloatObject}");
					ocl.assertQueryTrue(eObject,
						"negEFloatObjects(eFloatObjects) = Set{negEFloatObject(eFloatObject)}");
					ocl.assertQueryTrue(eObject, "eInts = Set{eInt}");
					ocl.assertQueryTrue(eObject,
						"negEInts(eInts) = Set{negEInt(eInt)}");
					ocl.assertQueryTrue(eObject,
						"eIntegerObjects = Set{eIntegerObject}");
					ocl.assertQueryTrue(eObject,
						"negEIntegerObjects(eIntegerObjects) = Set{negEIntegerObject(eIntegerObject)}");
					ocl.assertQueryTrue(eObject, "eLongs = Set{eLong}");
					ocl.assertQueryTrue(eObject,
						"negELongs(eLongs) = Set{negELong(eLong)}");
					ocl.assertQueryTrue(eObject,
						"eLongObjects = Set{eLongObject}");
					ocl.assertQueryTrue(eObject,
						"negELongObjects(eLongObjects) = Set{negELongObject(eLongObject)}");
					ocl.assertQueryTrue(eObject, "eShorts = Set{eShort}");
					ocl.assertQueryTrue(eObject,
						"negEShorts(eShorts) = Set{negEShort(eShort)}");
					ocl.assertQueryTrue(eObject,
						"eShortObjects = Set{eShortObject}");
					ocl.assertQueryTrue(eObject,
						"negEShortObjects(eShortObjects) = Set{negEShortObject(eShortObject)}");
					ocl.assertQueryTrue(eObject, "eBigInteger = eBigDecimal");
					// ocl.assertQueryTrue(eObject, "eNumber = eFloat"); --
					// waiting for BUG 370087
	...
	public void testEcoreTypes412736()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				// (-(b.oclAsType(Integer))).oclAsType(ENumber); }\n"
					...
					+ "        operation upCase(b : ecore::EString) : ecore::EString { body: b.toUpper(); }\n"
					+ "    }\n" + "}\n";
				String genmodelFile = createEcoreGenModelContent("Bug412736",
					null);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
					genmodelFile);
				...
				// ocl.assertQueryTrue(eObject, "eNumber = eFloat"); -- waiting
				// for BUG 370087
	...
	public void testEnumTypes412685()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				EPackage.Registry.INSTANCE.put(OCLstdlibPackage.eNS_URI,
				...
				EObject eObject = eFactory.create(eClass);
				ocl.assertQueryTrue(eObject,
					"let aWhite : OclAny = opaqueColor(eWhite) in eColor = aWhite");
				ocl.assertQueryTrue(eObject,
					"let aWhite : OclAny = eWhite.oclAsType(OclAny) in eColor = aWhite");
	...
	public void testSysML_QUDV()
		...
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				URI targetGenModelURI = getTestURI(
				...
				try {
					...
					//
					URI sourceGenModelURI = getTestModelURI(
						"models/genmodel/SysML_ValueTypes_QUDV.genmodel");
					Resource genModelResource = ocl0.getResourceSet()
						.getResource(sourceGenModelURI, true);
					GenModel genModel = (GenModel) genModelResource
						.getContents().get(0);
				...
				// getTestFileURI("SysML_ValueTypes_QUDV.ecore", ocl,
				// getTestModelURI("models/genmodel/SysML_ValueTypes_QUDV.ecore"));
				// URI targetGenModelURI =
				// getTestFileURI("SysML_ValueTypes_QUDV.genmodel", ocl,
				// getTestModelURI("models/genmodel/SysML_ValueTypes_QUDV.genmodel"));
				//
				doGenModel(targetGenModelURI);
				...
				try {
					...
					ExplicitClassLoader classLoader = new ExplicitClassLoader(
						classFilePath, packagePaths,
						getClass().getClassLoader());
					String qualifiedPackageName1 = testProjectName
						+ ".QUDV.QUDVPackage";
					EPackage ePackage1 = doLoadPackage(classLoader,
						qualifiedPackageName1);
					EClass eClass1 = (EClass) ePackage1
						.getEClassifier("DerivedQuantityKind");
					EFactory eFactory1 = ePackage1.getEFactoryInstance();
					//
					EObject eObject1 = eFactory1.create(eClass1);
					ocl.assertQueryTrue(eObject1,
						"dependsOnQuantityKinds() <> null");
					//
					String qualifiedPackageName2 = testProjectName
						+ ".PrimitiveValueTypes.PrimitiveValueTypesPackage";
					EPackage ePackage2 = doLoadPackage(classLoader,
						qualifiedPackageName2);
					EClass eClass2 = (EClass) ePackage2
						.getEClassifier("Complex");
					...
					ocl.assertQueryTrue(eObject2,
						"oclType().oclIsKindOf(OclAny)");
				} finally {
					ocl.dispose();
				}
			}
	 ...
	 * @throws Throwable
	 */
	public void testOpen_Pivot_oclas()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				if (EMFPlugin.IS_ECLIPSE_RUNNING) {
					...
					Resource resource = resourceSet1.getResource(
						URI.createURI(PivotPackage.eNS_URI, true), true);
					ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
					resource.setURI(URI.createPlatformResourceURI(
						testProjectName + "/" + "Pivot.oclas", true));
					...
					EcoreEditor openEditor = (EcoreEditor) IDE.openEditor(
						activePage, file,
						"org.eclipse.emf.ecore.presentation.ReflectiveEditorID",
						true);
					TestUIUtil.flushEvents();
					ResourceSet resourceSet = openEditor.getEditingDomain()
						.getResourceSet();
					EList<Resource> resources = resourceSet.getResources();
					assertEquals(1, resources.size());
					Resource resource2 = ClassUtil
						.nonNullState(resources.get(0));
	 ...
	 * @throws Throwable
	 */
	public void testOpen_Bug469251_uml()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				if (EMFPlugin.IS_ECLIPSE_RUNNING) {
					...
					TestUIUtil.flushEvents();
					//
					getTestFile("Bug469251.profile.uml", ocl,
						getTestModelURI("models/uml/Bug469251.profile.uml"));
					getTestFile("Bug469251.uml", ocl,
						getTestModelURI("models/uml/Bug469251.uml"));
					//
					IProject iProject = getTestProject().getIProject();
					IFile modelFile = iProject.getFile("Bug469251.uml");
					IWorkbenchPage activePage = workbench
						.getActiveWorkbenchWindow().getActivePage();
					UMLEditor umlEditor = (UMLEditor) IDE.openEditor(activePage,
						modelFile,
						"org.eclipse.uml2.uml.editor.presentation.UMLEditorID",
						true);
					TestUIUtil.flushEvents();
					/**
					 * This progresses the dialog but there is no clue as to
					 * what it did.
					 *
					 * String validateName =
					 * EMFEditUIPlugin.INSTANCE.getString("_UI_Validate_menu_item");
					 * IMenuManager menuManager =
					 * umlEditor.getActionBars().getMenuManager();
					 * IContributionManager validateItem1 =
					 * (IContributionManager)
					 * menuManager.findUsingPath("org.eclipse.uml2.umlMenuID");
					 * for (IContributionItem item : validateItem1.getItems()) {
					 * if (item instanceof ActionContributionItem){ IAction
					 * action = ((ActionContributionItem)item).getAction(); if
					 * (action.getText().equals(validateName)) { final Display
					 * display = Display.getCurrent(); display.timerExec(5000,
					 * new Runnable() { public void run() { Event event = new
					 * Event(); event.type = SWT.KeyDown; event.character =
					 * '\r'; display.post(event); } }); action.run(); break; } }
					 * }
					 */
					...
					Resource umlResource = ClassUtil
						.nonNullState(resources.get(0));
					...
					assertValidationDiagnostics("Validate", umlResource,
						getMessages(EcorePlugin.INSTANCE.getString(
							"_UI_GenericInvariant_diagnostic",
							new Object[]{"Constraint1",
								"«Stereotype1»" + LabelUtil.getLabel(xx)})));
					umlEditor.dispose();
				}
				ocl.dispose();
			}
	...
	public void testPivotMetamodelImport414855()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
			...
			public void runWithThrowable() throws Throwable {
				...
				genOptions.put("templateDirectory",
					"/org.eclipse.ocl.examples.codegen/templates");
				genOptions.put("usedGenPackages",
					"platform:/plugin/org.eclipse.ocl.pivot/model/Pivot.genmodel#//pivot");
				String genmodelFile = createEcoreGenModelContent(testFileStem,
					genOptions);
				createManifestFile();
				URI genModelURI = createModels(testFileStem, oclinecoreFile,
	...
	public void testTemplateTypes471201()
			throws Throwable {
		doTestRunnable(new TestRunnable() {
