			...
			if (le) {
				...
				try {
					...
					try {
						...
						catch (Exception THROWN_safe_isStatic_source) {
							CAUGHT_safe_isStatic_source = ValueUtil.createInvalidValue(THROWN_safe_isStatic_source);
						}
					...
					if (CAUGHT_not == ValueUtil.FALSE_VALUE) {
						...
						try {
							...
							if (conformsTo == Boolean.TRUE) {
								...
								@SuppressWarnings("null")
								final /*@Thrown*/ boolean conformsTo_0 = ((@NonNull Boolean)OclTypeConformsToOperation.INSTANCE.evaluate(executor, safe_type_source, getSpecializedReferredPropertyOwningType)).booleanValue();
