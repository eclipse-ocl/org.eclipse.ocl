...
//	}
	 ...
	 */
	private void mergeContainmentHierarchy(@NonNull Element mergedParent, @NonNull List<@NonNull ? extends Element> partialParents) {
		StringBuilder s = new StringBuilder();
		s.append("mergeContainmentHierarchy " + NameUtil.debugSimpleName(mergedParent));
		for (@NonNull Element partialParent : partialParents) {
			s.append("\n\t" + NameUtil.debugSimpleName(partialParent) + " : " + partialParent);
		}
		System.out.println(s.toString());
		if ((partialParents.get(0) instanceof Nameable) && "String".equals(((Nameable)partialParents.get(0)).getName())) {
			getClass();		// XXX
		}
					...
					if (eReference.isMany()) {
						...
						if (!Iterables.isEmpty(allPartialChildren)) {
							...
							for (@NonNull List<@NonNull Element> partialChildren : groupedPartialChildren.getOuterIterable()) {
								...
								mergeContainmentHierarchy(mergedChild, partialChildren);
							}
						}
					}
					else {
