...
public class EssentialOCLCSLeft2RightVisitor extends AbstractEssentialOCLCSLeft2RightVisitor
{
		...
		public PropertyScopeFilter(@NonNull List<SquareBracketedClauseCS> csSquareBracketedClauses) {
			...
			if (csSquareBracketedClauses.size() == 1) {
				...
				if (csTerms.size() == 1) {
					...
					NavigationCallExp navigationCallExp = PivotUtil.basicGetPivot(NavigationCallExp.class, csTerm);
	 ...
	 * @param isSafe
	 */
	protected @NonNull CallExp resolveExplicitSourceNavigation(@NonNull OCLExpression sourceExp, /*boolean isSafe,*/ @NonNull NameExpCS csNameExp) {
	 ...
	 * The return call has no body or return type since they cannot be synthesized until the body is synthesized.
	 */
	protected @Nullable IteratorExp resolveImplicitCollect(@NonNull OCLExpression sourceExp, @NonNull InfixExpCS csOperator) {		// XXX need excluding(null) if isSafe
		Type elementType;
...
//		}
	...
	//	int pivotAccumulatorsCount = 0;
		...
		for (NavigatingArgCS csArgument : csRoundBracketedClause.getOwnedArguments()) {
			...
			Variable acc = PivotUtil.basicGetPivot(Variable.class, csName);
			if (acc != null) {
				context.installPivotUsage(csArgument, acc);
				if (csInit != null) {
					OCLExpression initExpression = revisit(OCLExpression.class, csInit);
					acc.setOwnedInit(initExpression);
					TypedRefCS csAccType = csArgument.getOwnedType();
					Type accType = null;
					Boolean accIsRequired = null;
					if (csAccType != null) {
						accType = PivotUtil.basicGetPivot(Type.class, csAccType);
	...
	protected void resolveIterationBody(@NonNull RoundBracketedClauseCS csRoundBracketedClause, @NonNull LoopExp asLoopExp) {
		...
		for (NavigatingArgCS csArgument : csRoundBracketedClause.getOwnedArguments()) {
			if (csArgument.getRole() == NavigationRole.ITERATOR) {
				...
				assert name != null;
				OCLExpression asArgument = !hasIteratorOrAccumulator ? PivotUtil.basicGetPivot(OCLExpression.class, csArgument) : revisit(OCLExpression.class, name);
				if (asArgument == null) {
	...
	protected void resolveIterationIterators(@NonNull RoundBracketedClauseCS csRoundBracketedClause, @NonNull OCLExpression source, @NonNull LoopExp expression) {
		...
		for (int argIndex = 0; argIndex < csRoundBracketedClause.getOwnedArguments().size(); argIndex++) {
			...
			//			}
			ExpCS csName = csArgument.getOwnedNameExpression();
			Variable iterator = PivotUtil.basicGetPivot(Variable.class, csName);
			if (iterator != null) {
				...
				if (csCoIterator != null) {
					if ((pivotCoIterators != null)) {
						coIterator = PivotUtil.basicGetPivot(IteratorVariable.class, csCoIterator);
						if (coIterator != null) {
							hasCoIterator = true;
							...
							if (csCoIteratorType != null) {
								...
								coIteratorType = PivotUtil.basicGetPivot(Type.class, csCoIteratorType);
							}
	...
	protected void resolveOperationArgumentTypes(@Nullable List<Parameter> parameters, @NonNull RoundBracketedClauseCS csRoundBracketedClause) {
		...
		for (NavigatingArgCS csArgument : csRoundBracketedClause.getOwnedArguments()) {
			if (csArgument.getRole() == NavigationRole.ITERATOR) {
				...
				ExpCS csName = csArgument.getOwnedNameExpression();
				if (csName != null) {
					OCLExpression arg = revisit(OCLExpression.class, csName);
					if (arg != null) {
	...
	protected void resolveOperationArguments(@NonNull RoundBracketedClauseCS csRoundBracketedClause, @NonNull Operation operation, @NonNull OperationCallExp expression) {
		...
		if (csArgumentCount > 0) {
			...
			for (int argIndex = 0; argIndex < csArgumentCount; argIndex++) {
				...
				OCLExpression arg = PivotUtil.basicGetPivot(OCLExpression.class, csArgument);
				if (arg != null) {
					...
					if (argType != null) {
						...
						if (parameterType != null) {
							Operation asCoercion = resolveCoercionFrom(argType, arg.isIsRequired(), parameterType, parameter.isIsRequired());
							if (asCoercion != null) {
	...
	protected void resolveOperationCall(@NonNull OperationCallExp expression, @NonNull OperatorExpCS csOperator) {
		...
		OCLExpression ownedSource = PivotUtil.getOwnedSource(expression);
		Type sourceType = PivotUtil.getType(ownedSource);
		Operation asOperation = null;
		Invocations invocations = null;
		if (!AUTOGENERATED_LOOKUP) {
			if (name != null) {
				if (sourceType instanceof NormalizedTemplateParameter) {
					sourceType = getCS2ASContext().resolveTemplateParameter((NormalizedTemplateParameter)sourceType);
				}
		...
		if (invocations != null) {
			...
			if ((csOperator instanceof InfixExpCS) && !NavigationUtil.isNavigationInfixExp(csOperator)) {	// explicit: X op Y
				matcher = new BinaryOperationMatcher(getCS2ASContext(), sourceType, ((InfixExpCS) csOperator).getArgument());
			}
			else {																	// explicit: op X, or implicit: X.oclAsSet()->
				matcher = new UnaryOperationMatcher(getCS2ASContext(), sourceType);
			}
	...
	protected void resolveOperationReturnType(@NonNull CallExp callExp) {
		Operation operation = PivotUtil.basicGetReferredOperation(callExp);
		if (operation == null) {
			return;
		}
		...
		if (callExp instanceof OperationCallExp) {
			List<@NonNull Parameter> parameters = PivotUtil.getOwnedParametersList(operation);
			List<@NonNull OCLExpression> arguments = PivotUtil.getOwnedArgumentsList((OperationCallExp)callExp);
			int iMax = Math.min(parameters.size(), arguments.size());
			for (int i = 0; i < iMax; i++) {
				Parameter parameter = parameters.get(i);
				OCLExpression argument = arguments.get(i);
				Type parameterType = PivotUtil.getTypeInternal(parameter);
			...
			Parameter accumulator = PivotUtil.getOwnedAccumulator((Iteration)operation);
			Variable result = PivotUtil.getOwnedResult(iterateExp);
			Type accumulatorType = PivotUtil.getTypeInternal(accumulator);
	...
	protected Element resolveRoundBracketedTerm(@NonNull RoundBracketedClauseCS csRoundBracketedClause) {
		...
		if (NavigationUtil.isNavigationInfixExp(csParent) && (csParent != null) && (csNameExp != csParent.getSource())) {
			...
			return PivotUtil.basicGetPivot(OCLExpression.class, csNameExp);
		}
	...
	protected <T extends Element> @Nullable T revisit(@NonNull Class<T> pivotClass, @NonNull ElementCS csElement) {
		...
		//	System.out.println("visit " + csElement.eClass().getName() + " " + csElement);		// XXX
			...
			context.addError(csElement, message);
		}
		if (element == null) {
			return null;
		}
	...
	public Element visitBooleanLiteralExpCS(@NonNull BooleanLiteralExpCS csBooleanLiteralExp) {
		BooleanLiteralExp expression = PivotUtil.basicGetPivot(BooleanLiteralExp.class, csBooleanLiteralExp);
		if (expression != null) {
	...
	public Element visitCollectionLiteralExpCS(@NonNull CollectionLiteralExpCS csCollectionLiteralExp) {
		...
		for (CollectionLiteralPartCS csPart : csCollectionLiteralExp.getOwnedParts()) {
			assert csPart != null;
			CollectionLiteralPart pivotPart = revisit(CollectionLiteralPart.class, csPart);
		...
		//			return invalidValue;
		//		}
		CollectionLiteralExp expression = PivotUtil.basicGetPivot(CollectionLiteralExp.class, csCollectionLiteralExp);
		if (expression != null) {
	...
	public Element visitCollectionLiteralPartCS(@NonNull CollectionLiteralPartCS csCollectionLiteralPart) {
		...
		OCLExpression pivotFirst = revisit(OCLExpression.class, csFirst);
		OCLExpression pivotLast = null;
		ExpCS csLast = csCollectionLiteralPart.getOwnedLastExpression();
		if (csLast == null) {
			CollectionItem expression = PivotUtil.basicGetPivot(CollectionItem.class, csCollectionLiteralPart);
			if (expression != null) {
				expression.setOwnedItem(pivotFirst);
			}
		}
		else {
			CollectionRange expression = PivotUtil.basicGetPivot(CollectionRange.class, csCollectionLiteralPart);
			if (expression != null) {
				expression.setOwnedFirst(pivotFirst);
				pivotLast = revisit(OCLExpression.class, csLast);
		...
		CollectionLiteralPart expression = PivotUtil.basicGetPivot(CollectionLiteralPart.class, csCollectionLiteralPart);
		if (expression != null) {
	...
	public Element visitContextCS(@NonNull ContextCS csContext) {
		ExpressionInOCL pivotElement = PivotUtil.basicGetPivot(ExpressionInOCL.class, csContext);
		if (pivotElement != null) {
			...
			if (csExpression != null) {
				...
				OCLExpression expression = revisit(OCLExpression.class, csExpression);
				if (expression != null) {
	...
	public Element visitIfExpCS(@NonNull IfExpCS csIfExp) {
		IfExp ifExpression = PivotUtil.basicGetPivot(IfExp.class, csIfExp);
		ExpCS csElse = csIfExp.getOwnedElseExpression();
		if ((ifExpression != null) && (csElse != null)) {
			OCLExpression elseExpression = revisit(OCLExpression.class, csElse);
			...
			for (int i = csIfThens.size(); --i >= 0; ) {
				...
				IfExp elseIfExpression = PivotUtil.basicGetPivot(IfExp.class, csIfThen);
	...
	public Element visitInfixExpCS(@NonNull InfixExpCS csInfixExp) {
		...
		if (csRoot != currentRoot) {
			...
			try {
				currentRoot = csRoot;
				OCLExpression pivot = revisit(OCLExpression.class, csRoot);
	...
	protected @NonNull OCLExpression doIfThenElse(@NonNull IfExp expression, @Nullable ExpCS csCondition, @Nullable ExpCS csThen, @Nullable OCLExpression elseExpression) {
		if ((csCondition != null) && (csThen != null)) {
			expression.setOwnedCondition(revisit(OCLExpression.class, csCondition));
			OCLExpression thenExpression = revisit(OCLExpression.class, csThen);
	...
	protected OCLExpression doVisitNavigationOperatorCS(@NonNull InfixExpCS csOperator) {
		...
		ExpCS csSource = csOperator.getSource();
		if (csSource != null) {
			OCLExpression sourceExp = revisit(OCLExpression.class, csSource);
			if (sourceExp != null) {
				...
				if (actualSourceType != null) {
					...
					if (argument instanceof NameExpCS) {
						...
						//
						//	Normalize the source for implicit oclAsSet or implicit collect
						//
						...
						if (sourceIsAggregate && PivotUtil.isObjectNavigationOperator(navigationOperatorName)) {
							...
							if (implicitCollectExp != null) {
								@NonNull Variable iterator = PivotUtil.getOwnedIterator(implicitCollectExp, 0);
								normalizedSourceExp = createImplicitVariableExp(iterator);
							}
						}
						else if (!sourceIsAggregate && PivotUtil.isAggregateNavigationOperator(navigationOperatorName)) {
							normalizedSourceExp = resolveImplicitAsSet(sourceExp, actualSourceType, csOperator);
						}
						Type normalizedSourceType = normalizedSourceExp.getType();
						csNameExp.setSourceType(normalizedSourceType);
						csNameExp.setSourceTypeValue(normalizedSourceExp.getTypeValue());
						//
	...
	public Element visitInvalidLiteralExpCS(@NonNull InvalidLiteralExpCS csInvalidLiteralExp) {
		InvalidLiteralExp expression = PivotUtil.basicGetPivot(InvalidLiteralExp.class, csInvalidLiteralExp);
		if (expression == null) {
			expression = helper.createInvalidExpression();
		}
	...
	public Element visitLetExpCS(@NonNull LetExpCS csLetExp) {
		...
		for (LetVariableCS csLetVariable : csLetExp.getOwnedVariables()) {
			LetVariable variable = PivotUtil.basicGetPivot(LetVariable.class, csLetVariable);
			if (variable != null) {
				LetExp letExp;
				...
				if (csInitExpression != null) {
					...
					if (csVariableType != null) {
						variableType = PivotUtil.basicGetPivot(Type.class, csVariableType);
					...
					if (initExpression != null) {
						...
						if ((initType != null) && (variableType != null)) {
							boolean variableIsRequired2 = variableIsRequired != null ? variableIsRequired.booleanValue() : initIsRequired;
							Operation asCoercion = resolveCoercionFrom(initType, initIsRequired, variableType, variableIsRequired2);
							if (asCoercion != null) {
		...
		if (lastLetExp != null) {
			...
			if (csIn != null) {
				OCLExpression in = revisit(OCLExpression.class, csIn);
	...
	public Element visitMapLiteralExpCS(@NonNull MapLiteralExpCS csMapLiteralExp) {
		...
		for (MapLiteralPartCS csPart : csMapLiteralExp.getOwnedParts()) {
			assert csPart != null;
			MapLiteralPart pivotPart = revisit(MapLiteralPart.class, csPart);
			if (pivotPart != null) {
				if (inferKeyType) {
		...
		MapLiteralExp expression = PivotUtil.basicGetPivot(MapLiteralExp.class, csMapLiteralExp);
		if (expression != null) {
	...
	public Element visitMapLiteralPartCS(@NonNull MapLiteralPartCS csMapLiteralPart) {
		...
		OCLExpression asKey = revisit(OCLExpression.class, csKey);
		OCLExpression asValue = revisit(OCLExpression.class, csValue);
		MapLiteralPart expression = PivotUtil.basicGetPivot(MapLiteralPart.class, csMapLiteralPart);
		if (expression != null) {
	...
	public Element visitNameExpCS(@NonNull NameExpCS csNameExp) {
		...
		if ((element == null) || element.eIsProxy()) {
			...
			InvalidLiteralExp invalidLiteralExp = helper.createInvalidExpression();
	...
	public Element visitNavigatingArgCS(@NonNull NavigatingArgCS csNavigatingArg) {
		OCLExpression pivot = PivotUtil.basicGetPivot(OCLExpression.class, csNavigatingArg);
		if (pivot != null) {
	...
	public Element visitNestedExpCS(@NonNull NestedExpCS csNestedExp) {
		...
		OCLExpression pivot = revisit(OCLExpression.class, csSource);
		if (pivot != null) {
	...
	public Element visitNullLiteralExpCS(@NonNull NullLiteralExpCS csNullLiteralExp) {
		NullLiteralExp expression = PivotUtil.basicGetPivot(NullLiteralExp.class, csNullLiteralExp);
		if (expression != null) {
	...
	public Element visitNumberLiteralExpCS(@NonNull NumberLiteralExpCS csNumberLiteralExp) {
		NumericLiteralExp expression = PivotUtil.basicGetPivot(NumericLiteralExp.class, csNumberLiteralExp);
	...
	public Element visitSelfExpCS(@NonNull SelfExpCS csSelfExp) {	// FIXME Just use VariableExpCS
		VariableExp expression = PivotUtil.basicGetPivot(VariableExp.class, csSelfExp);
		if (expression != null) {
	...
	public Element visitShadowPartCS(@NonNull ShadowPartCS csShadowPart) {
		ShadowPart pivotElement = PivotUtil.basicGetPivot(ShadowPart.class, csShadowPart);
		if (pivotElement != null) {
	...
	public Element visitStringLiteralExpCS(@NonNull StringLiteralExpCS csStringLiteralExp) {
		StringLiteralExp pivotElement = PivotUtil.basicGetPivot(StringLiteralExp.class, csStringLiteralExp);
		if (pivotElement != null) {
	...
	public Element visitTupleLiteralExpCS(@NonNull TupleLiteralExpCS csTupleLiteralExp) {
		TupleLiteralExp expression = PivotUtil.basicGetPivot(TupleLiteralExp.class, csTupleLiteralExp);
		if (expression != null) {
			...
			assert parts != null;
			Type type = standardLibrary.getTupleType(parts, null);
			helper.setType(expression, type, true, null);
		}
		return expression;
	}
	...
	public Element visitTupleLiteralPartCS(@NonNull TupleLiteralPartCS csTupleLiteralPart) {
		TupleLiteralPart pivotElement = PivotUtil.basicGetPivot(TupleLiteralPart.class, csTupleLiteralPart);
		if (pivotElement != null) {
			...
			if (csInitExpression != null) {
				...
				Type type = csType != null ? PivotUtil.basicGetPivot(Type.class, csType) : initExpression != null ? initExpression.getType() : null;
	...
	public Element visitTypeLiteralExpCS(@NonNull TypeLiteralExpCS csTypeLiteralExp) {
		...
		//		visitInOrder(csType, null);
		Type type = PivotUtil.basicGetPivot(Type.class, csType);
	...
	public Element visitUnlimitedNaturalLiteralExpCS(@NonNull UnlimitedNaturalLiteralExpCS csUnlimitedNaturalLiteralExp) {
		UnlimitedNaturalLiteralExp expression = PivotUtil.basicGetPivot(UnlimitedNaturalLiteralExp.class, csUnlimitedNaturalLiteralExp);
		if (expression != null) {
			helper.setType(expression, standardLibrary.getUnlimitedNaturalType(), true, null);
		}
		return expression;
	...
	public Element visitVariableCS(@NonNull VariableCS csVariable) {
		...
		if (variable != null) {
			OCLExpression initExpression = PivotUtil.basicGetPivot(OCLExpression.class, csVariable.getOwnedInitExpression());
			if (initExpression != null) {
				Type initType = initExpression.getType();
				TypedRefCS csType = csVariable.getOwnedType();
				Type type;
				if (csType != null) {
					type = PivotUtil.basicGetPivot(Type.class, csType);
				}
				else {
					type = initType;
