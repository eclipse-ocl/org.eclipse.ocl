...
import org.eclipse.ocl.pivot.utilities.Invocations.UnresolveableInvocations;
...
public class EssentialOCLCSLeft2RightVisitor extends AbstractEssentialOCLCSLeft2RightVisitor
{
	...
	/**
	 * Return an example operation from the invocations prior to valifdation of argument lists of static sources.
	...
	protected @Nullable Invocations getInvocations(@Nullable Type asSourceType, boolean hasExplicitSourceExp, @NonNull RoundBracketedClauseCS csRoundBracketedClause) {
		...
		if ((asElement instanceof Operation) && !asElement.eIsProxy()) {
			return new Invocations.ResolvedInvocation((Operation)asElement);
		}
		...
		if (pathSize > 1) {											// Search for B::b() or a.B::b() candidates in B
			...
			Invocations invocations = getInvocations(asType, hasExplicitSourceExp, name, iteratorCount, expressionCount);
			if ((invocations == null) && name.startsWith("_")) {
				@NonNull String unescapedName = name.substring(1);				// FIXME Compatibility
				invocations = getInvocations(asType, hasExplicitSourceExp, unescapedName, iteratorCount, expressionCount);
			}
			return invocations;
		}
			...
			Invocations invocations = getInvocations(asSourceType, hasExplicitSourceExp, name, iteratorCount, expressionCount);
			if ((invocations == null) && name.startsWith("_")) {
				String unescapedName = name.substring(1);				// FIXME Compatibility
				invocations = getInvocations(asSourceType, hasExplicitSourceExp, unescapedName, iteratorCount, expressionCount);
			}
			return invocations;
		}
			...
			for (ImplicitSourceTypeIterator it = createImplicitSourceTypeIterator(csNameExp); (invocations == null) && it.hasNext(); ) {
				Type asType = it.next();
				invocations = getInvocations(asType, hasExplicitSourceExp, name, iteratorCount, expressionCount);
			}
			if ((invocations == null) && name.startsWith("_")) {
				String unescapedName = name.substring(1);				// FIXME Compatibility
				for (ImplicitSourceTypeIterator it = createImplicitSourceTypeIterator(csNameExp); (invocations == null) && it.hasNext(); ) {
	...
	/**
	 * Return all operations/iterations in sourceType and its superclasses whose name is name. For iterations the number of iteration iterators must
	 ...
	 * nothing is found. A first invocation search looks for non-static operations or iterations that may use the source type
	 * of an explicit source expression or that pplicable to an implicit source/self.
	 * If that fails and if not hasExplicitSourceExp a further search of static operations ismade.
	 */
	protected @Nullable Invocations getInvocations(@NonNull Type sourceType, boolean hasExplicitSourceExp, @NonNull String name, int iteratorCount, int expressionCount) {
		int colonIndex = name.indexOf(":");
		if (colonIndex >= 0) {
			String languageName = name.substring(0, colonIndex);
			String qualifiedOperationName = name.substring( colonIndex+1);
			LanguageSupport languageSupport = environmentFactory.getLanguageSupport(languageName);
			if (languageSupport == null) {
				return new UnresolveableInvocations(PivotMessages.NoLanguageSupport, languageName);
			}
			return languageSupport.resolveInvocations(sourceType, hasExplicitSourceExp, qualifiedOperationName);
		}
		TemplateParameter asTemplateParameter = sourceType.isTemplateParameter();
		if (asTemplateParameter != null) {
		...
		if (sourceType instanceof ElementExtension) {				// FIXME review me
			...
			if (asStereotype != null) {
				Iterable<@NonNull Operation> stereotypeOperations = metamodelManager.getAllOperations(asStereotype, FeatureFilter.SELECT_NON_STATIC, name);
				invocations = getInvocationsInternal(invocations, stereotypeOperations, iteratorCount, expressionCount);
			}
		}
		if (!hasExplicitSourceExp) {
			Iterable<@NonNull Operation> staticOperations = metamodelManager.getAllOperations(sourceType, FeatureFilter.SELECT_STATIC, name);
	...
	protected @Nullable OCLExpression resolveBestInvocation(@Nullable OCLExpression sourceExp, @NonNull RoundBracketedClauseCS csRoundBracketedClause, @NonNull Invocations invocations) {
		boolean hasExplicitSourceExp = sourceExp != null;
		AbstractNameExpCS csNameExp = csRoundBracketedClause.getOwningNameExp();
		PathNameCS csPathName = csNameExp.getOwnedPathName();
		if (csPathName == null) {
			return null;
			...
			for (org.eclipse.ocl.pivot.Class partialClass : completeClass.getPartialClasses()) {
				if (partialClass instanceof PrimitiveType) {
					for (Operation coercion : ((PrimitiveType)partialClass).getCoercions()) {
						...
						Invocations coercedInvocations = getInvocations(corcedSourceType, hasExplicitSourceExp, csRoundBracketedClause);
	...
	protected @Nullable IteratorExp resolveImplicitCollect(@NonNull OCLExpression sourceExp, @NonNull InfixExpCS csOperator) {
		...
		Invocations invocations = getInvocations(actualSourceType, true, "collect", 1, 0);
		if (invocations == null) {
			return null;
		}
	...
	protected @NonNull OCLExpression resolveInvocation(@Nullable OCLExpression sourceExp, @NonNull RoundBracketedClauseCS csRoundBracketedClause) {
		...
		Invocations invocations = getInvocations(sourceExp != null ? sourceExp.getType() : null, sourceExp != null,
			csRoundBracketedClause);
		if (invocations != null) {
			String unresolveableReason = invocations.getUnresolveableReason();
			if (unresolveableReason != null) {
				context.addError(csPathName != null ? csPathName : csNameExp, unresolveableReason);
			}
			else {
	...
	protected void resolveOperationCall(@NonNull OperationCallExp expression, @NonNull OperatorExpCS csOperator) {
		...
		OCLExpression sourceExp = PivotUtil.getOwnedSource(expression);
		Type sourceType = PivotUtil.getType(sourceExp);
		Operation asOperation = null;
		Invocations invocations = null;
		if (!AUTOGENERATED_LOOKUP) {
			if (name != null) {
				invocations = getInvocations(sourceType, true, name, 0, expression.getOwnedArguments().size());
				if ((invocations == null) && name.startsWith("_")) {
					String unescapedName = name.substring(1);				// FIXME Compatibility
					invocations = getInvocations(sourceType, true, unescapedName, 0, expression.getOwnedArguments().size());
				}
			}
