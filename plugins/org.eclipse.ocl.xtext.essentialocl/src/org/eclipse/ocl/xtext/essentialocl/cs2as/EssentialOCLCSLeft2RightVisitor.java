...
//		}
	...
	protected StateExp resolveStateExp(@NonNull ExpCS csExp, @NonNull State state) {
		...
		helper.setType(expression, ((EnvironmentFactoryExtension2)environmentFactory).getASClass("State"), true, null);		// FIXME What should this be
	...
	public Element visitTupleLiteralPartCS(@NonNull TupleLiteralPartCS csTupleLiteralPart) {
		...
		if (pivotElement != null) {
			...
			if (csInitExpression != null) {
				...
				Boolean partIsRequired = csType != null ? context.getConverter().isRequired(csType) : null;		// Explicit part declaration dominates init inference
				helper.setType(pivotElement, type, partIsRequired != null ? partIsRequired : initExpression != null ? initExpression.isIsRequired() : false);
			}
		}
		return pivotElement;
	}
